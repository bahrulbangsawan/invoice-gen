...
import type { CVData, ExperienceEntry, EducationCategory } from "@/components/cv-form"
...
/** Pure function: apply a single action to CVData and return new data */
function applyOneAction(data: CVData, action: ApplyAction): CVData {
  switch (action.section) {
    case "personal-info": {
      ...
      return { ...data, personalInfo: { ...data.personalInfo, ...updates } }
    }

    case "summary":
      return { ...data, summary: action.content }

    case "experience": {
      if (action.index !== undefined && data.experience[action.index]) {
      ...
      if (action.index === undefined) {
        ...
        for (const line of rawLines) {
          ...
          if (pipeCount >= 6) {
            ...
            expLines[expLines.length - 1] = `${expLines[expLines.length - 1]}\n${line.trim()}`
          }
        }
        ...
        return { ...data, experience: [...existingExp.values()] }
      }
      return data
    }
    ...
    case "skills": {
      ...
      const existingMap = new Map(
        data.skills.map((cat) => [cat.name.toLowerCase(), cat]),
      )
      ...
      return { ...data, skills: [...existingMap.values()] }
    }

    case "languages": {
      const langLines = action.content.split("\n").filter((l) => l.trim())
      const existingMap = new Map(
        data.languages.map((l) => [l.language.toLowerCase(), l]),
      )
      ...
      return { ...data, languages: [...existingMap.values()] }
    }

    case "awards": {
      const awardLines = action.content.split("\n").filter((l) => l.trim())
      const existingAwards = new Map(
        data.awards.map((a) => [a.title.toLowerCase(), a]),
      )
      ...
      return { ...data, awards: [...existingAwards.values()] }
    }

    case "certificates": {
      const certLines = action.content.split("\n").filter((l) => l.trim())
      const existingCerts = new Map(
        data.certificates.map((c) => [c.name.toLowerCase(), c]),
      )
      ...
      return { ...data, certificates: [...existingCerts.values()] }
    }
    ...
    case "education": {
      ...
      for (const line of eduLines) {
        ...
        existingEdu.set(key, {
          id: existing?.id ?? crypto.randomUUID(),
          category: (parts[5] ?? existing?.category ?? "") as EducationCategory,
          degree,
          institution,
      ...
      return { ...data, education: [...existingEdu.values()] }
    }

    case "projects": {
      const portLines = action.content.split("\n").filter((l) => l.trim())
      const existingPort = new Map(
        data.projects.map((p) => [p.name.toLowerCase(), p]),
      )
      ...
      return { ...data, projects: [...existingPort.values()] }
    }
    ...
    case "volunteer": {
      ...
      return { ...data, volunteer: [...existingVol.values()] }
    }
...
export function CVAssistant({ data, onApply }: CVAssistantProps) {
  ...
  const buildSystemPromptWithMention = useCallback(
    (sections?: CVSectionKey[]) => buildSystemPrompt(data, sections),
    [data],
  )
