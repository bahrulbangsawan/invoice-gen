                ...
                {experience.filter((e) => e.company || e.title).map((entry) => (
                  <section key={entry.id} className="mt-4">
                    ...
                    <p className="text-xs text-neutral-700">
                      {entry.company && (entry.url ? <a href={ensureUrl(entry.url)} target="_blank" rel="noopener noreferrer" className="underline">{entry.company}</a> : entry.company)}
            ...
            return (
              <section key={sectionId} className="mt-6">
                ...
                {certificates.filter((e) => e.name).map((entry) => (
                  <section key={entry.id} className="mt-4">
                    <h3 className="text-sm font-bold text-neutral-700">{entry.url ? <a href={ensureUrl(entry.url)} target="_blank" rel="noopener noreferrer" className="underline">{entry.name}</a> : entry.name}</h3>
                    <p className="text-xs text-neutral-700">
